...
def composite_ps5_header_4k(image_data: bytes) -> bytes:
    """Composites the 4K official PS5 master header cleanly onto full-bleed artwork."""
    with Image.open(io.BytesIO(image_data)) as raw_img:
        ...
        # 1. Resize artwork to fit full-bleed below header
        ...
        # 2. Paste 4K Master Clean Header
...
def select_best_base_product(items: list) -> dict:
    """Intelligently filters out all DLCs/packs and returns ONLY the true base game."""
    if not items:
        return None
    ...
    for item in items:
        ...
        # Check if name contains disqualified DLC/pack keywords
...
def extract_best_image_url(product: dict) -> str:
    """Extracts the premier full-cover portrait artwork from a product."""
    images = product.get("localization", {}).get("images", [])
    
    # Priority order for official retail artwork
    # 1. PORTRAIT_BANNER (Official vertical box art)
    for img in images:
        if img.get("type") == "PORTRAIT_BANNER":
            return img.get("url")
            
    # 2. EDITION_KEY_ART (Official edition key art)
    for img in images:
        if img.get("type") == "EDITION_KEY_ART":
            return img.get("url")
            
    # 3. GAMEHUB_COVER_ART (Official gamehub cover art)
    for img in images:
        if img.get("type") == "GAMEHUB_COVER_ART":
            return img.get("url")
            
    # 4. Fallback to any first image
...
async def process_title(session: aiohttp.ClientSession, ppsa_id: str, semaphore: asyncio.Semaphore):
    async with semaphore:
        try:
            ...
            print(f"✅ 4K Base Game: {ppsa_id}.webp -> {prod_name[:40]}")
            return True
        except Exception:
            return False
...
async def main(start_id: int = 1, end_id: int = 10000, concurrency: int = 40):
    ...
    print(f"🛡️ Ejecutando Scraper Inteligente de Juegos Base ({len(ppsa_ids)} títulos)...")
    
    semaphore = asyncio.Semaphore(concurrency)
    async with aiohttp.ClientSession(headers={"User-Agent": "Mozilla/5.0 MacheteSmart/2.0"}) as session:
        tasks = [process_title(session, pid, semaphore) for pid in ppsa_ids]
        results = await asyncio.gather(*tasks)
        success = sum(1 for r in results if r)
        print(f"\n🎉 ¡Proceso Finalizado con Éxito: {success} juegos base garantizados subidos a Cloudflare R2!")
